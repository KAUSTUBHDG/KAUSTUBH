    ...
    public static class PlayersInfo
    {
        ...
        public static void Initialize(List<Character> characters)
        {
            ...
            if (!playersInfoFile.Exists)
            {
                ...
                using (StreamWriter sw = playersInfoFile.CreateText())
                {
                    ...
                    {
                        sw.WriteLine($"{character.Name},{character._scores},{character.Level}");
                    }
                }
            }
            ...
            for (int i = 0; i < characters.Count; i++)
            {
                for (int j = 0; j < characters.Count; j++)
                {
                    ...
                    {
                        characters[i]._scores = Convert.ToInt32(fullInfo[j][1]);
        ...
        public static void Save(List<Character> characters)
        {
            using (StreamWriter sw = playersInfoFile.CreateText())
            {
                foreach (var character in characters)
                {
                    sw.WriteLine(string.Join(",", $"{character.Name},{character._scores},{character.Level}"));
                }
            }
        }
        ...
        public static void UpdateFullInfo(List<Character> characters)
        {
            ...
            {
                fullInfo[i] = ($"{characters[i].Name},{characters[i]._scores},{characters[i].Level}").Split(',');
            }
        }
